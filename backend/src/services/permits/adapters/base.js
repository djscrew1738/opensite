...
export class BaseAdapter {
  constructor(source, logger) {
    ...
    this.fieldMapping = source.fieldMapping ? JSON.parse(source.fieldMapping) : {};
  ...
  async run(daysBack = 7) {
    this.logger.info(`[${this.name}] Starting permit fetch (${daysBack} days back)...`);

    const rawRecords = await this.fetchRawPermits(daysBack);
    this.logger.info(`[${this.name}] Fetched ${rawRecords.length} raw records`);
    ...
    for (const raw of rawRecords) {
      try {
        ...
        record.permitCategory = this.classifyPermitType(record.permitType, record.description);
        normalized.push(record);
      } catch (err) {
        this.logger.warn(`[${this.name}] Failed to normalize record: ${err.message}`, {
          raw_id: raw.permit_number || raw.id || 'unknown'
        });
      }
    }

    this.logger.info(`[${this.name}] Normalized ${normalized.length}/${rawRecords.length} records`);
    return normalized;
  }
