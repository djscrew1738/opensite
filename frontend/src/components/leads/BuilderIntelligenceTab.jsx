      ...
      <div className="grid grid-cols-3 gap-3">
        <div className="card">
          <div className="card-body p-5 text-center">
            ...
            <p className="text-2xl font-bold text-surface-900 dark:text-surface-100 tabular-nums">{stats.total}</p>
            <p className="text-2xs font-bold uppercase tracking-wider text-surface-500">Builders</p>
          </div>
        </div>
        <div className="card">
          <div className="card-body p-5 text-center">
            <TrendingUp className="w-5 h-5 text-emerald-500 mx-auto mb-1" />
            <p className="text-2xl font-bold text-emerald-600 dark:text-emerald-400 tabular-nums">{stats.rising}</p>
            <p className="text-2xs font-bold uppercase tracking-wider text-surface-500">Rising</p>
          </div>
        </div>
        <div className="card">
          <div className="card-body p-5 text-center">
            <AlertCircle className="w-5 h-5 text-hot-500 mx-auto mb-1" />
            <p className="text-2xl font-bold text-hot-600 dark:text-hot-400 tabular-nums">{stats.noPlumber}</p>
            <p className="text-2xs font-bold uppercase tracking-wider text-surface-500">No Plumber</p>
          </div>
        </div>
      </div>
              ...
              <button
                ...
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold transition-all ${
                  trendFilter === f.key
                    ...
                    : 'bg-concrete-100 dark:bg-surface-800 text-surface-500 dark:text-surface-400 hover:bg-concrete-200 dark:hover:bg-surface-700'
                }`}
              >
                {f.label}
              </button>
          ...
          <div className="card-body p-5">
            ...
            <div className="flex gap-2 overflow-x-auto pb-1 scrollbar-hide">
              {prospects.slice(0, 5).map(p => (
                ...
                >
                  <p className="text-sm font-bold text-surface-900 dark:text-surface-100 whitespace-nowrap">{p.name || p.company}</p>
                  <p className="text-xs text-surface-500 mt-0.5">{p.totalPermits || 0} permits · {p.activityTrend}</p>
                </button>
              ))}
            </div>
          </div>
      ...
      {isLoading ? (
        ...
        <div className="card">
          <div className="card-body text-center py-16">
            ...
            <h3 className="text-xl font-bold text-surface-900 dark:text-surface-100 mb-2">No builders found</h3>
