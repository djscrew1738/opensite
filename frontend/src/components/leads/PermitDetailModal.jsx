    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4" onClick={onClose}>
      ...
      >
        ...
        <div className={`bg-gradient-to-r ${tierColors[permit.leadTier] || tierColors.cold} p-6 rounded-t-2xl`}>
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-white text-4xl font-bold">{permit.leadScore}</p>
            </div>
        ...
        <div className="p-6 space-y-5">
          ...
          {/* Permit Type */}
          <div>
            <p className="text-lg font-semibold text-surface-800 dark:text-surface-200">{permit.permitType}</p>
          </div>

          {/* Key Details Grid */}
          <div className="grid grid-cols-2 gap-4">
            <div className="p-3 bg-concrete-50 dark:bg-surface-800 rounded-xl">
              <div className="flex items-center gap-2 text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wide mb-1">
            ...
            {permit.estimatedCost && (
              <div className="p-3 bg-concrete-50 dark:bg-surface-800 rounded-xl">
                ...
                <p className="text-sm font-medium text-surface-900 dark:text-surface-100">{formatCurrency(permit.estimatedCost)}</p>
              </div>
            )}
            ...
            {permit.issuedDate && (
              <div className="p-3 bg-concrete-50 dark:bg-surface-800 rounded-xl">
                ...
                <p className="text-sm font-medium text-surface-900 dark:text-surface-100">{formatDate(permit.issuedDate)}</p>
              </div>
            )}
            ...
            {permit.permitNumber && (
              <div className="p-3 bg-concrete-50 dark:bg-surface-800 rounded-xl">
                ...
                <p className="text-sm font-medium text-surface-900 dark:text-surface-100 font-mono">{permit.permitNumber}</p>
              </div>
            )}
          </div>
            ...
            <div className="flex gap-4">
              {permit.units && (
                <div className="flex-1 p-3 bg-blue-50 dark:bg-blue-950/20 rounded-xl text-center">
                  <p className="text-2xl font-bold text-blue-700 dark:text-blue-400">{permit.units}</p>
              ...
              {permit.squareFootage && (
                <div className="flex-1 p-3 bg-accent-50 dark:bg-accent-950/20 rounded-xl text-center">
                  <p className="text-2xl font-bold text-accent-700 dark:text-accent-400">{permit.squareFootage.toLocaleString()}</p>
          ...
          {permit.description && (
            <div>
              <p className="text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wide mb-1">Description</p>
              <p className="text-sm text-surface-700 dark:text-surface-300 leading-relaxed">{permit.description}</p>
            </div>
          )}
          ...
          {(permit.contractorPhone || permit.contractorEmail) && (
            <div className="space-y-2">
              <p className="text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wide">Contact</p>
          ...
          {/* Status Progression */}
          <div>
            <p className="text-xs font-semibold text-surface-500 dark:text-surface-400 uppercase tracking-wide mb-2">Status</p>
            <StatusProgressBar
