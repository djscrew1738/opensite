...
export default function ConfirmDialog({ title, message, confirmLabel = 'Delete', onConfirm, onCancel, variant = 'danger' }) {
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[60] p-4" onClick={onCancel}>
      <div
        className="bg-white dark:bg-surface-900 rounded-2xl max-w-md w-full shadow-2xl border border-concrete-200 dark:border-surface-700 animate-slide-up"
        ...
        <div className="p-6 space-y-4">
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-sm text-surface-600 dark:text-surface-400 mt-1">{message}</p>
            </div>
            <button onClick={onCancel} className="text-surface-400 hover:text-surface-600 dark:hover:text-surface-300 shrink-0">
              <X className="w-5 h-5" />
            </button>
          </div>
