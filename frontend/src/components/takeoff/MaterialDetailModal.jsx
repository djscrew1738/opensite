        ...
        {isLoading ? (
          <div className="px-6 py-8">
            <div className="animate-pulse space-y-3">
              <div className="h-6 bg-surface-200 rounded w-2/3"></div>
            ...
            <div className="px-6 py-4 border-b border-surface-200">
              <div className="flex items-start justify-between">
                <div className="flex-1 min-w-0">
                  ...
                  <h2 className="text-xl font-bold text-surface-900">{material.name}</h2>
                  {material.description && (
                    <p className="text-sm text-surface-600 mt-1">{material.description}</p>
                  )}
                </div>
            ...
            <div className="flex-1 overflow-y-auto px-6 py-4 space-y-5">
              ...
              <div className="bg-surface-50 rounded-xl p-4">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-xs text-surface-500 uppercase tracking-wider font-semibold mb-1">Unit Cost</p>
                    <div className="flex items-baseline gap-2">
                      <span className="text-3xl font-bold text-surface-900">
                        ${Number(material.unitCost).toFixed(2)}
                      </span>
                      <span className="text-sm text-surface-500">per {material.unit}</span>
                    </div>
                    {priceWithMarkup && (
              ...
              {material.notes && (
                <div>
                  <h4 className="text-xs text-surface-500 uppercase tracking-wider font-semibold mb-2 flex items-center gap-1.5">
                    <FileText className="w-3.5 h-3.5" />
                    Notes
                  </h4>
                  <div className="bg-surface-50 rounded-lg p-3 text-sm text-surface-700 whitespace-pre-wrap">
                    {material.notes}
                  </div>
                </div>
              )}
              ...
              {history.length > 0 && (
                <div>
                  ...
                  <div className="mt-3 bg-white border border-surface-200 rounded-lg overflow-hidden">
                    <table className="w-full text-sm">
                      <thead>
                        <tr className="bg-surface-50 border-b border-surface-200">
                          <th className="text-left px-3 py-2 text-surface-600 font-medium">Date</th>
                          <th className="text-right px-3 py-2 text-surface-600 font-medium">Old Price</th>
                          <th className="text-right px-3 py-2 text-surface-600 font-medium">New Price</th>
                          <th className="text-right px-3 py-2 text-surface-600 font-medium">Change</th>
                        </tr>
                      </thead>
                          ...
                          return (
                            <tr key={entry.id} className="hover:bg-surface-50">
                              ...
                              <td className={`px-3 py-2 text-right font-medium ${
                                change > 0 ? 'text-red-600' : change < 0 ? 'text-green-600' : 'text-surface-500'
                              }`}>
                    ...
                    {history.length > 10 && (
                      <div className="px-3 py-2 bg-surface-50 text-xs text-surface-500 text-center">
          ...
          <div className="p-8 text-center text-surface-500">
            <Package className="w-12 h-12 mx-auto mb-2 text-surface-300" />
  ...
  return (
    <div className="flex items-start gap-3">
      <div className="w-8 h-8 bg-surface-100 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5">
        <IconComponent className="w-4 h-4 text-surface-500" />
      </div>
      <div>
        <p className="text-xs text-surface-500">{label}</p>
        <p className={`text-sm font-medium text-surface-900 ${mono ? 'font-mono' : ''}`}>{value}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="bg-white border border-surface-200 rounded-lg p-3">
      ...
      <div className="flex items-center justify-between text-xs text-surface-500 mt-1">
        ...
        <span className="text-surface-400">{prices.length} data points</span>
        <span className="font-medium text-surface-900">${prices[prices.length - 1].toFixed(2)}</span>
      </div>
    </div>
  );
}
